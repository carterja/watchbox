...
const THUMB_SLIDE =
  "min-w-0 shrink-0 basis-[min(56vw,168px)] sm:basis-[188px] md:basis-[220px]";

/** Main panel: fast flicks can skip snaps; smooth deceleration feels closer to iOS. */
const MAIN_EMBLA_OPTS = {
...
/** Thumbnail strip: momentum scrolling through many titles without fighting snap. */
const THUMB_EMBLA_OPTS = {
  align: "center" as const,
  axis: "x" as const,
  dragFree: true,
...
export function WatchingNextCarousel({
  ...
  const [emblaMainRef, emblaMainApi] = useEmblaCarousel(MAIN_EMBLA_OPTS);
  const [emblaThumbsRef, emblaThumbsApi] = useEmblaCarousel(THUMB_EMBLA_OPTS);
  ...
  useEffect(() => {
    ...
    emblaMainApi.on("select", syncThumbs);
    emblaMainApi.on("reInit", syncThumbs);
    syncThumbs();
    return () => {
      emblaMainApi.off("select", syncThumbs);
      emblaMainApi.off("reInit", syncThumbs);
    };
  }, [emblaMainApi, emblaThumbsApi]);

  // Thumbnails (user drag) → main — use settle so dragFree drag does not fight scroll snap mid-gesture
  useEffect(() => {
    if (!emblaThumbsApi || !emblaMainApi) return;
    const syncMain = () => {
      const idx = emblaThumbsApi.selectedScrollSnap();
      if (emblaMainApi.selectedScrollSnap() !== idx) {
        emblaMainApi.scrollTo(idx);
      }
    };
    emblaThumbsApi.on("settle", syncMain);
    return () => {
      emblaThumbsApi.off("settle", syncMain);
    };
  ...
  }, [rowsKey, emblaMainApi, emblaThumbsApi]);

  const scrollSnaps = emblaMainApi?.scrollSnapList() ?? [];
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      >
        ...
        {/* Thumbnail strip — drag-free momentum */}
        <div className="overflow-hidden pb-2 pt-1 md:pt-2" ref={emblaThumbsRef}>
          <div className="flex gap-4 md:gap-5">
            {rows.map((row, index) => {
              const src = row.posterPath ? posterUrl(row.posterPath, "w342") : null;
              const isActive = index === clampedIndex;
              return (
                <div key={row.mediaId} className={THUMB_SLIDE}>
                  <button
                    type="button"
        ...
        {snapCount > 1 && (
          <div
            className="watching-embla-dots flex justify-center gap-1.5 pb-2 pt-1 md:pb-3"
            aria-hidden
          >
            {rows.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${
                  i === clampedIndex
                    ? "w-8 bg-[#8b5cf6] shadow-[0_0_12px_rgba(139,92,246,0.45)]"
                    : "w-1.5 bg-[#8b5cf6]/35 hover:bg-[#8b5cf6]/55"
                }`}
      ...
      {/* Main: hero + detail — swipe anywhere on this card */}
      <div className="overflow-hidden" ref={emblaMainRef}>
        <div className="flex">
          {rows.map((row) => (
            <div
              key={row.mediaId}
              className="min-w-0 shrink-0 grow-0 basis-full pl-0 pr-0"
            >
              <WatchingNextDetailSlide
                row={row}
                marking={marking}
                onMarkWatched={onMarkWatched}
                onOpenSetPosition={onOpenSetPosition}
...
function WatchingNextDetailSlide({
  row,
  marking,
  onMarkWatched,
  onOpenSetPosition,
  ...
  const activeStream =
    resolveStreaming ? resolveStreaming(row.mediaId) : null;
  const detailPosterSrc =
    row.posterPath != null ? posterUrl(row.posterPath, "w342") : null;
  const episodeStillSrc =
    row.next?.stillPath != null ? posterUrl(row.next.stillPath, "w500") : null;
  const primaryVisualSrc = episodeStillSrc ?? detailPosterSrc;
  const nextAirLabel = row.next?.airDate != null ? formatAirDate(row.next.airDate) : null;
  const activeMedia = resolveMedia ? resolveMedia(row.mediaId) : null;
  const checklistProgress = seasonChecklistProgress(activeMedia);

  return (
    <div className="relative mx-auto max-w-3xl overflow-hidden rounded-2xl border border-white/[0.08] bg-[#121218] shadow-[0_24px_48px_-12px_rgba(0,0,0,0.55)] ring-1 ring-white/[0.04]">
        <div
            ...
            <div className="absolute left-3 top-3">
              <span className="inline-flex rounded-full bg-fuchsia-400 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-black shadow-lg">
                {activeMedia?.status === "finished"
                  ? `S${row.next.season} complete`
                  : `S${row.next.season} ongoing`}
              </span>
            </div>
          )}
        ...
        <div className="relative space-y-4 p-4 sm:p-5 md:p-6">
            ...
            {row.lastFinished && (
              <p className="text-[11px] text-shelf-muted/90">
                ...
                <span className="tabular-nums text-shelf-muted">
                  S{row.lastFinished.season}E{row.lastFinished.episode}
                </span>
              </p>
            )}
          </div>
          ...
          {row.next && (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-shelf-muted">
              ...
              {row.next.runtimeMinutes != null && (
                <span className="inline-flex items-center gap-1.5">
                  <Clock size={14} className="shrink-0 text-shelf-muted/70" aria-hidden />
                  <span className="tabular-nums text-white/85">{row.next.runtimeMinutes} min</span>
                </span>
              )}
            </div>
          )}

          {row.caughtUp && !row.next ? (
            ...
            <div className="rounded-2xl border border-white/[0.08] bg-shelf-card/80 p-4 shadow-inner">
              <div className="flex items-start justify-between gap-3">
                ...
                <p className="shrink-0 text-xs tabular-nums text-shelf-muted">
                  S{row.next.season} · E{row.next.episode}
                </p>
              </div>
          ...
          <div className="flex flex-col gap-3 border-t border-white/[0.08] pt-4">
            <div className="flex flex-wrap items-stretch justify-center gap-2 sm:justify-start">
              {row.next && (
                <button
                  type="button"
                  onClick={() => void onMarkWatched(row.mediaId)}
                  disabled={marking === row.mediaId}
