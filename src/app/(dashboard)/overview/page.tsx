...
function QueueRow({ row }: { row: WhatNextRow }) {
  ...
  return (
    <li className="flex gap-3 rounded-xl border border-shelf-border bg-shelf-card/40 p-3">
      ...
      <Link
        href={href}
        className="self-center shrink-0 rounded-lg border border-shelf-border px-2.5 py-1.5 text-xs text-shelf-accent hover:bg-shelf-card"
      >
        <span className="inline-flex items-center gap-1">
          Series <ExternalLink size={12} />
        </span>
      </Link>
    </li>
  );
